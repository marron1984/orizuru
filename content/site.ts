/**
 * ORIZURU ティザーサイト コピー定数
 *
 * 表記ルール（厳守）:
 * - FSUN は「国連支援財団（Foundation for the Support of the United Nations / FSUN）」と表記。
 *   国連（UN）そのものではない。国連公認／認定等の表現は使わない。FSUN と連携 までに留める。
 * - かぎ括弧（鉤括弧）は本文コピーで使わない。強調はトーンと装飾で表現する。
 * - 一人称は組織主体として 私たち を使う。
 * - 当事者・子どもは、自らの意思で世界へ働きかける主体として描く。
 * - 投資・節税・出資・利回り・台数・銀行名などの資金スキームは一切含めない。
 */

export const site = {
  name: "ORIZURU",
  tagline: "千羽鶴が、世界を救う。",
  // metadata（SEO / OGP）— 投資系キーワードは含めない
  meta: {
...
export const hero = {
  ...
  scrollCue: "読み進める",
  // 注記: ヒーローの人物画像は実在の利用者写真ではなくイメージ素材を用いる前提。
