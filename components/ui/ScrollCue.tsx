...
export function ScrollCue({ label }: { label: string }) {
  ...
  return (
    <a
      href="#problem"
      className="group inline-flex flex-col items-center gap-2 text-paper/70 transition-colors hover:text-paper"
    >
      <span className="text-xs uppercase tracking-kicker">{label}</span>
      <motion.span
        aria-hidden
        className="flex h-9 w-5 items-start justify-center rounded-full border border-paper/40 p-1"
